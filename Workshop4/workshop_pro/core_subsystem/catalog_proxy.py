...
class CatalogProxy:
    ...
    def add_vehicle(self, username: str):
        ...
        Observability.write_user_log(
            username, "A new vehicle had been added."
        )

    def remove_vehicle(self, username: str):
        """This method removes a vehicle from the catalog."""
        self.__catalog.remove_vehicle()
        Observability.write_user_log(
            username, "Delete a vehicle"
        )

    def get_all_vehicles(self, username: str):
        """This method gets all vehicles from the catalog."""
        for vehicle in self.__catalog.get_all_vehicles():
            print(str(vehicle))
        Observability.write_user_log(
            username, "Search all vehicles"
        )

    def get_vehicles_by_speed(self, username: str):
        ...
        Observability.write_user_log(
            username, "Search vehicles for speed"
        )

    def get_vehicles_by_price(self, username: str):
        ...
        Observability.write_user_log(
            username, "Search Vechicles for price"
        )
        
    def get_last_five(self):
        return self.__catalog.get_last_five()
